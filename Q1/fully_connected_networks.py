...
class Linear(object):
    ...
    def forward(x, w, b):
        ...
        out =  torch.mm(x_reshaped,w.to(torch.float64)) + b
    ...
    def backward(dout, cache):
        ...
        ##################################################
        dout = dout.to(torch.float64)
            # Compute gradients
        dx = torch.mm(dout, w.t()).view(x.shape)
        dw = torch.mm(x.view(x.shape[0], -1).t().to(torch.float64),dout)
...
class Linear_ReLU(object):
    ...
    def forward(x, w, b):
        ...
        # relu_cache =  torch.mm(x_reshaped,w) + b.t()
        # cache = (x, w, b, relu_cache) 
        # out = torch.max(relu_cache,torch.zeros(relu_cache.shape)) 
    ...
    def backward(dout, cache):
        ...
        ######################################################################
        # x, w, b, relu_cache = cache     # relu_cache  >>    XW + B
        
        # relu_cache[relu_cache>0] = 1
        # relu_cache[relu_cache<0] = 0
        #     # Compute gradients
        # dx = torch.mm(dout*relu_cache, w.t()).view(x.shape)
        # dw = torch.mm(x.view(x.shape[0], -1).t(),dout*relu_cache)
        # db = torch.sum(dout*relu_cache, axis=0)
...
def softmax_loss(x, y):
    ...
    N = x.shape[0]
    loss = -torch.log(probs_[range(N), y]).mean()
...
class TwoLayerNet(object):
    ...
    def __init__(self, input_dim=3*32*32, hidden_dim=100, num_classes=10,
        ...
        ###################################################################
        self.params['W1'] = weight_scale* torch.randn(size=(input_dim,hidden_dim)).to(torch.float64)
        self.params['b1'] =  torch.zeros(size=(1,hidden_dim)).to(torch.float64)
        self.params['W2'] = weight_scale* torch.randn(size=(hidden_dim,num_classes)).to(torch.float64)
        self.params['b2'] = torch.zeros(size=(1,num_classes)).to(torch.float64)
        ###############################################################
        #                            END OF YOUR CODE                 #
        ###############################################################
    ...
    def loss(self, X, y=None):
        ...
        # scores variable.                                          #
        #############################################################
        # linear - relu - linear - softmax >>>  Softmax(Relu(XW1+b1).W2 + b2)
        ...
        layer1, _ = relu.forward(torch.mm(X,self.params['W1']) + self.params['b1'])
        scores = torch.mm(layer1,self.params['W2']) + self.params['b2'] 
        
        # out =  softmax(scores)  # output of network
        ...
        h1 = layer1
        dw2 = torch.mm(h1.t(),dx)
        dh1 = torch.mm(dx,self.params['W2'].t())
        dz1 = deepcopy(dh1)
        dz1 = torch.max(dh1,torch.zeros(dh1.shape,device=dh1.device))
        dw1 = torch.mm(X.t(), dz1)
        
        db2 = torch.sum(dx,dim= 0) 
        db1 = torch.sum(dz1,dim= 0) 
        
        grads['W1'] = dw1 + 2*self.reg*self.params['W1']
        grads['W2'] = dw2 + 2*self.reg*self.params['W2']
        grads['b1'] = db1  + 2*self.reg*self.params['b1']
        grads['b2'] = db2  + 2*self.reg*self.params['b2']
        
        
        loss += self.reg* (torch.sum(torch.square(self.params['W1'])) +
                           torch.sum(torch.square(self.params['W2'])))
                          #  torch.sum(torch.square(self.params['b1']))+
                          #  torch.sum(torch.square(self.params['b2'])))
...
class FullyConnectedNet(object):
    ...
    def __init__(self, hidden_dims, input_dim=3*32*32, num_classes=10,
        ...
        for layer in range(self.num_layers):
            ...
            self.params[f'W{layer+1}'] = weight_scale* torch.randn(size=(dims[layer], dims[layer + 1])).to(torch.float64)
            self.params[f'b{layer+1}'] = torch.zeros(dims[layer + 1], dtype=dtype, device=device).to(torch.float64)
    ...
    def loss(self, X, y=None): 
        ...
        # X = X.view(X.shape[0],-1)
        # scores, _ = relu.forward(torch.mm(X,self.params['W1']) + self.params['b1'])
        
        # for i in range(2,self.N):
        #     scores,_ = relu.forward(torch.mm(scores,self.params[f'W{i}']) + self.params[f'b{i}'])
        
        # scores = torch.mm(scores,self.params[f'W{self.N}']) + self.params[f'b{self.N}']
        # # out =  softmax(scores)  # output of network
        
        # forward pass 
        
        caches = []  # save cashes for backward move
        fc_out = X
        
        for layer in range (self.num_layers) :

            if layer != self.num_layers-1: # in hidden layer
                fc_out , cache = Linear_ReLU.forward(fc_out , self.params[f'W{layer+1}'] , self.params[f'b{layer+1}'])
                
                if self.use_dropout:   # using dropout
                    fc_out, cache_drop = Dropout.forward(fc_out, self.dropout_param)
                    caches.append((cache, cache_drop))
                else:
                    caches.append(cache)
                                    
            else :  # in output layer
                fc_out , cache = Linear.forward(fc_out , self.params[f'W{layer+1}'] ,self.params[f'b{layer+1}'])
                caches.append(cache)
        ...
        for layer in range(self.num_layers-1,-1,-1):
            ...
            if layer != self.num_layers - 1: # # in hidden layer
                if self.use_dropout:
                    cache, cache_drop = caches[layer]
                    dA = Dropout.backward(dA, cache_drop)
                else:
                    cache = caches[layer]
                    
                dA, dw, db = Linear_ReLU.backward(dA, cache)
                grads[f'W{layer + 1}'] = dw + 2 *self.reg *self.params[f'W{layer + 1}']
            ...
            loss += self.reg * torch.sum(self.params[f'W{layer + 1}'] ** 2)  # L2-regularization effect on loss
...
def rmsprop(w, dw, config=None):
    ...
    # config['cache'].                                                        #
    ###########################################################################
    # Update squared gradients  >>>   squared_gradients = beta * squared_gradients + (1 - beta) * gradients^2
    config['cache'] = (config['cache']*config['decay_rate']) + (1 - config['decay_rate']) * (dw ** 2)
    
    # Update parameters  >>>  theta = theta - (learning_rate / sqrt(squared_gradients + epsilon)) * gradients
...
def adam(w, dw, config=None):
    ...
    ##########################################################################
    config['t'] = config['t'] + 1 # update timesetp
    
    config['m'] = config['beta1'] * config['m'] + (1 - config['beta1']) * dw  # update 1'st momentom
    m_hat = config['m'] / (1 - config['beta1'] **config['t']) # bias correction
    
    config['v'] = config['beta2'] * config['v'] + (1 - config['beta2']) * (dw ** 2) # update 2'nd momentom
    v_hat = config['v'] / (1 - config['beta2'] **config['t']) # bias correction
    
    next_w = w - config['learning_rate'] * m_hat / (torch.sqrt(v_hat) + config['epsilon'])  # optimization
    #########################################################################
    #                              END OF YOUR CODE                         #
    #########################################################################
    return next_w, config
...
class Dropout(object):
    ...
    def forward(x, dropout_param):
        ...
        if mode == 'train':
            ...
            ##############################################################
            # `torch.rand_like(x)` function in PyTorch is used to create a tensor with 
            # the same shape as the input tensor `x`, filled with
            # random values from a uniform distribution between 0 and 1.
